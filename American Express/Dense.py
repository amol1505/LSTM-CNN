...
#displays data during years for american express from yahoo finance
stock_df
...
#calculating rows to train the model on, first 80% of dataset (~2010-2018) = training set
training_datarows = int(np.ceil(len(dataset) * .80))
# returns number of rows
training_datarows

#scaling the data to normalise it between values 0-1
...
#sliding window algorithm to use previous 60 days of data to predict value, turns into supervised learning problem
...
#reshaping data into 3d array for input
X_train = np.reshape(X_train, (X_train.shape[0], X_train.shape[1], 1))
        
from keras.models import Sequential 
from keras.layers import Dense, Conv1D, LSTM, Bidirectional, MaxPooling1D, Flatten, Dropout
...
history = model.fit(X_train, Y_train, batch_size=1, epochs=6)
print(history.history.keys())
plt.plot(history.history['loss'])
plt.title('MLP Loss over training')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.show()

#testing data will be last 20% of closing prices in dataset
...
plt.title('MLP Stock Forecast')
plt.xlabel('Date', fontsize=13)
plt.ylabel('AXP Close Price USD ($)', fontsize=13)
